...
void setupSteeringControl() {
  ...
  steerServo.setPeriodHertz(50);    // Standard 50 Hz servo (20ms period)
...
void handleSteeringControl() {
  ...
  if (abs(Ps3.event.analog_changed.stick.rx) > MV_JOYSTICK_DEADZONE) {
    ...
    // SAFETY: Validate servo position is within bounds
    joystick_Pos = constrain(joystick_Pos, servoMin, servoMax);
